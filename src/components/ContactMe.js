...
export const ContactMe = () => {
      ...
      const handleSubmitEmail = (e) => {
            ...
            // Config Email JS
            const serviceID = 'service_sfdtu4t';
            const templateID = 'template_y6341b2';
            const publicKey = 'ezKtrQcB2LqeE9IXp';

            // Config template email
            const templateEmail = {
                  from_name: inputData.name,
                  from_email: inputData.email,
                  to_name: 'Diva Indira Asauri',
                  message: inputData.message,
            };

            emailjs.send(serviceID,templateID,templateEmail,publicKey).then((res) => {
                  console.log("Email Berhasil Dikirim : " + res);
                  setInputData({
                        name:"",
                        email:"",
                        message:""
      ...
      const handleInput = (e) => {
            ...
            if(nameInput === "name"){
                  setInputData({...inputData, name: valueInput});
            } else if(nameInput === "email"){
                  setInputData({...inputData, email: valueInput});
            } else if(nameInput === "message"){
                  setInputData({...inputData, message: valueInput});
            }
      };
                        ...
                        <div className="xl:mt-32 xl:px-10">
                              <div className="grid grid-cols-12 lg:gap-4 xl:gap-14">
                                    ...
                                    <div className="col-span-12 mt-10 px-4 md:px-16 md:mt-20 lg:col-span-6 lg:px-0 lg:self-start">
                                          ...
                                          <form onSubmit={handleSubmitEmail} className="flex flex-col gap-3 mt-8 xl:gap-6 xl:mt-10">
                                                <div aria-label="group-input" className="flex flex-col gap-2 xl:gap-4">
                                                      ...
                                                      <input value={inputData.name} onChange={handleInput} type="text" name="name" placeholder="Silahkan Masukan Nama Bapak/Ibu" className="w-full bg-[#FAF1F1] py-2 pl-3 placeholder:text-[0.6rem] md:py-4 md:pl-6 md:text-[1rem] md:placeholder:text-[1rem] placeholder:text-[#A9A9AA99] placeholder:font-normal focus:outline-none focus:border-orange-web focus:ring-2 focus:ring-orange-web xl:rounded-[0.16rem] xl:py-6 xl:pl-7 xl:placeholder:tracking-tight"/>
                                                </div>
                                                <div aria-label="group-input" className="flex flex-col gap-2 xl:gap-4">
                                                      <label htmlFor="" className="font-normal text-[0.6rem] md:text-[1rem] xl:text-[1.12rem]">Alamat Email*</label>
                                                      <input value={inputData.email} onChange={handleInput} type="email" name="email" placeholder="Silahkan Masukan Email Bapak/Ibu" className="w-full bg-[#FAF1F1] py-2 pl-3 placeholder:text-[0.6rem] md:py-4 md:pl-6 md:text-[1rem] md:placeholder:text-[1rem] placeholder:text-[#A9A9AA99] placeholder:font-normal focus:outline-none focus:border-orange-web focus:ring-2 focus:ring-orange-web xl:rounded-[0.16rem] xl:py-6 xl:pl-7 xl:placeholder:tracking-tight"/>
                                                </div>
                                                <div aria-label="group-input" className="flex flex-col gap-2 xl:gap-4">
                                                      <label htmlFor="" className="font-normal text-[0.6rem] md:text-[1rem] xl:text-[1.12rem]">Kirim Pesan</label>
                                                      <textarea value={inputData.message} onChange={handleInput} type="text" name="message" placeholder="Silahkan Masukan Apa Yang Ingin Disampaikan" className="w-full bg-[#FAF1F1] pt-1 pb-12 pl-3 placeholder:text-[0.6rem] md:pt-4 md:pl-6 md:pb-16 md:placeholder:text-[1rem] md:text-[1rem] lg:pt-3 placeholder:text-[#A9A9AA99] placeholder:font-normal focus:outline-none focus:border-orange-web focus:ring-2 focus:ring-orange-web xl:rounded-[0.16rem] xl:pt-6 xl:pb-14 xl:pl-7 xl:placeholder:tracking-tight"></textarea>
                                                </div>
